...
GENERATION_PROMPT_USD_PER_1M_TOKENS=0
GENERATION_COMPLETION_USD_PER_1M_TOKENS=0
...
Nếu cần hiển thị ước tính chi phí, điền hai biến
`GENERATION_PROMPT_USD_PER_1M_TOKENS` và
`GENERATION_COMPLETION_USD_PER_1M_TOKENS` theo bảng giá provider tại thời điểm
chạy. Mặc định để `0` để tránh hard-code giá cloud đã thay đổi.
...
2. Backend extract text, clean text, dựng section tree rule-based và tách chunk theo section.
3. Mỗi chunk được gửi riêng cho DeepSeek API theo 2 bước: trích knowledge point rồi sinh câu hỏi JSON.
4. Candidate được kiểm tra deterministic rule, trích dẫn nguồn, one-best-answer/answerability bằng LLM validation và duplicate bằng E5 local.
5. Job lưu model, prompt version, token usage, latency, trạng thái partial và lỗi theo chunk để retry.
6. Người dùng duyệt/sửa/từ chối candidate rồi mới lưu vào ngân hàng câu hỏi.
7. Khi lưu, hệ thống tạo embedding E5 và cập nhật local vector index.
