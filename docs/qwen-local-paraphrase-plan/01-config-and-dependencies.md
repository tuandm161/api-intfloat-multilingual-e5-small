...
- If Windows installation fails, document the fallback path using a local server later; do not change the default implementation away from GGUF in-app.
...
local_paraphrase_max_tokens: int = 512
local_paraphrase_temperature: float = 0.35
